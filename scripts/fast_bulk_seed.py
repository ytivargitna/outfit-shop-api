...
def main():
    print('Connecting to Neon PostgreSQL Database...', flush=True)
    ...
    print('1. Resetting all catalog tables...', flush=True)
    cur.execute('''
        TRUNCATE TABLE 
            product_images,
            product_variants,
    ...
    print('2. Seeding Clothing Sizes & Colors...', flush=True)
    size_values = [
        ("('Extra Small (XS)', 'Chest 34 inch', NOW(), NOW())"),
        ("('Small (S)', 'Chest 36 inch', NOW(), NOW())"),
        ("('Medium (M)', 'Chest 38 inch', NOW(), NOW())"),
        ("('Large (L)', 'Chest 40 inch', NOW(), NOW())"),
        ("('Extra Large (XL)', 'Chest 42 inch', NOW(), NOW())"),
        ("('Double Extra Large (XXL)', 'Chest 44 inch', NOW(), NOW())"),
        ("('One Size (OS)', 'Universal / Free Size', NOW(), NOW())")
    ]
    cur.execute(f"INSERT INTO clothing_sizes (size_name, description, created_at, updated_at) VALUES {','.join(size_values)};")
    cur.execute("SELECT size_name, size_id FROM clothing_sizes;")
    size_map = dict(cur.fetchall())

    color_values = [
        ("('Classic Black', '#000000', NOW(), NOW())"),
        ("('Pure White', '#FFFFFF', NOW(), NOW())"),
        ("('Navy Blue', '#000080', NOW(), NOW())"),
        ("('Charcoal Gray', '#4A4A4A', NOW(), NOW())"),
        ("('Vintage Green', '#2E8B57', NOW(), NOW())"),
        ("('Crimson Red', '#DC143C', NOW(), NOW())"),
        ("('Sand / Beige', '#F5F5DC', NOW(), NOW())")
    ]
    cur.execute(f"INSERT INTO colors (color_name, description, created_at, updated_at) VALUES {','.join(color_values)};")
    cur.execute("SELECT color_id FROM colors WHERE color_name = 'Classic Black';")
    color_black_id = cur.fetchone()[0]

    print('3. Seeding 24 Brands...', flush=True)
    brand_map = {}
    for folder_slug, meta in BRAND_METADATA.items():
        cur.execute(f'''
            ...
            VALUES ({sql_escape(meta['name'])}, {sql_escape(folder_slug.lower())}, {sql_escape(meta['origin'])}, {sql_escape(meta['desc'])}, {sql_escape(meta['featured'])}, NOW(), NOW())
            RETURNING brand_id;
        ''')
        b_id = cur.fetchone()[0]
        brand_map[folder_slug] = (b_id, meta['name'])

    print('4. Seeding 15 Categories...', flush=True)
    cat_map = {}
    for c_name, c_slug, dept, desc in CATEGORIES:
        cur.execute(f'''
            ...
            VALUES ({sql_escape(c_name)}, {sql_escape(c_slug.lower())}, {sql_escape(dept)}, {sql_escape(desc)}, NOW(), NOW())
    ...
    print('5. Loading Cloudinary Live URL map...', flush=True)
    ...
    print(f'Loaded {len(cloudinary_url_map)} Cloudinary CDN URLs.', flush=True)

    print('6. Grouping and Preparing Products from Desktop/product-items...', flush=True)
    products_to_insert = []
    products_metadata = []
    ...
    for brand_folder, (brand_id, brand_name) in brand_map.items():
        ...
        for cat_folder in sorted(os.listdir(b_path)):
            ...
            for prod_title, rel_files in product_groups.items():
                ...
                row_sql = f"({cat_id}, {brand_id}, 'PHYSICAL_APPAREL', {sql_escape(prod_title)}, {sql_escape(brand_name)}, '{gender}', '100% Premium Cotton / Technical Blend', 'Annual Collection 2026', {sql_escape(f'Official {brand_name} {prod_title} crafted with authentic materials.')}, {sql_escape(primary_url)}, {sql_escape(badge)}, 'ACTIVE', NOW(), NOW())"
                products_to_insert.append(row_sql)
    ...
    print(f'7. Bulk inserting {len(products_to_insert)} products in chunks...', flush=True)
    inserted_product_ids = []

    for i in range(0, len(products_to_insert), 150):
        chunk = products_to_insert[i:i+150]
        cur.execute(f"INSERT INTO products (category_id, brand_id, product_type, product_name, brand, gender, material_fabric, season_collection, description, image_url, featured_badge, status, created_at, updated_at) VALUES {','.join(chunk)} RETURNING product_id;")
        ids = [row[0] for row in cur.fetchall()]
        inserted_product_ids.extend(ids)

    print(f'Successfully inserted {len(inserted_product_ids)} products!', flush=True)

    print('8. Generating Variants and Image SQL records...', flush=True)
    variant_rows = []
    image_rows = []
    ...
    for idx, p_id in enumerate(inserted_product_ids):
        ...
        for s_name, s_code in size_list:
            ...
            variant_rows.append(f"({p_id}, {s_id}, {color_black_id}, {sql_escape(sku)}, {sql_escape(barcode)}, {cost_p}, {base_p}, {whole_p}, {qty}, 10, {sql_escape(meta['primary_url'])}, NOW(), NOW())")
        ...
        for f_idx, rel_path in enumerate(meta['rel_files']):
            ...
            is_primary = 'true' if (f_idx == 0) else 'false'
            sort_order = f_idx + 1
            alt = f"{meta['title']} angle {sort_order}"

            image_rows.append(f"({p_id}, {sql_escape(img_cdn_url)}, {is_primary}, {sort_order}, {sql_escape(alt)}, NOW(), NOW())")

    print(f'9. Bulk inserting {len(variant_rows)} product variants in chunks...', flush=True)
    for i in range(0, len(variant_rows), 200):
        chunk = variant_rows[i:i+200]
        cur.execute(f"INSERT INTO product_variants (product_id, size_id, color_id, sku, barcode, cost_price, sale_price, wholesale_price, quantity, reorder_level, image_url, created_at, updated_at) VALUES {','.join(chunk)};")

    print(f'10. Bulk inserting {len(image_rows)} product gallery images in chunks...', flush=True)
    for i in range(0, len(image_rows), 200):
        chunk = image_rows[i:i+200]
        cur.execute(f"INSERT INTO product_images (product_id, image_url, is_primary, sort_order, alt_text, created_at, updated_at) VALUES {','.join(chunk)};")

    print('\n========================================================', flush=True)
    print('DATABASE SEEDING COMPLETE WITH 100% RELATIONAL INTEGRITY!', flush=True)
    print('========================================================', flush=True)
    print(f' • Total Brands: {len(brand_map)}', flush=True)
    print(f' • Total Categories: {len(CATEGORIES)}', flush=True)
    print(f' • Total Clothing Sizes: {len(size_map)}', flush=True)
    print(f' • Total Colors: {len(color_values)}', flush=True)
    print(f' • Total Products: {len(inserted_product_ids)}', flush=True)
    print(f' • Total Product Variants: {len(variant_rows)}', flush=True)
    print(f' • Total Product Images: {len(image_rows)}', flush=True)
